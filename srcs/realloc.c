...
void	*start_realloc(void *ptr, size_t size)
{
	...
	if (isset_addr(ptr) == 0)
		return (NULL);
	return (copy_in_new_malloc_and_free((t_block_mem *)(ptr - sizeof(t_block_mem)), ptr, size));
	if (size == 0)
	{
		start_free(ptr);
		return (start_malloc(0));
	}
	...
	return (copy_in_new_malloc_and_free(mem, ptr, size)); // to remove
